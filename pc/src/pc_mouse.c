/* pc_mouse.c - Mouse input for menus and other 2D interfaces.
 *
 * Sampled once per frame by pc_mouse_update() at the end of the SDL event
 * pump, so every reader in a frame sees the same snapshot: button edges,
 * wheel movement, motion, and a position mapped into GC screen space. */

#include "pc_platform.h"
#include "pc_mouse.h"

#ifdef MOUSE_INPUT

static s32 g_mouse_x = 0, g_mouse_y = 0;
static s32 g_mouse_dx = 0, g_mouse_dy = 0;
static s32 g_mouse_locked = 0;
static s32 g_mouse_moved = 0;
static u32 g_mouse_buttons_current = 0;
static u32 g_mouse_buttons_previous = 0;
static int g_mouse_active_this_frame = 0;

/* Accumulated by the SDL event loop between updates. */
s32 g_mouse_wheel_delta = 0;
/* This frame's latched wheel movement, cleared once a reader consumes it. */
static s32 g_mouse_wheel_frame = 0;

u32 pc_mouse_button_held(void) {
    return g_mouse_buttons_current;
}

u32 pc_mouse_button_pressed(void) {
    return (g_mouse_buttons_current & ~g_mouse_buttons_previous);
}

u32 pc_mouse_button_released(void) {
    return (~g_mouse_buttons_current & g_mouse_buttons_previous);
}

void pc_mouse_lock(s32 lock) {
    g_mouse_locked = !!lock;
    SDL_SetRelativeMouseMode(g_mouse_locked ? SDL_TRUE : SDL_FALSE);
}

int pc_mouse_is_locked(void) {
    return g_mouse_locked;
}

s32 pc_mouse_scroll_wheel(void) {
    s32 wheel = g_mouse_wheel_frame;
    g_mouse_wheel_frame = 0;
    return wheel;
}

void pc_mouse_get_position(s32* x, s32* y) {
    if (x) *x = g_mouse_x;
    if (y) *y = g_mouse_y;
}

void pc_mouse_get_delta(s32* dx, s32* dy) {
    if (dx) *dx = g_mouse_dx;
    if (dy) *dy = g_mouse_dy;
}

int pc_mouse_moved(void) {
    return g_mouse_moved;
}

int pc_mouse_active(void) {
    return g_mouse_active_this_frame;
}

#ifdef PC_ENHANCEMENTS
/* Pillarbox correction applied by pc_gx.c when the window is wider than 4:3. */
extern float g_aspect_factor;
extern float g_aspect_offset;
extern int   g_aspect_active;
#endif

/* Window pixels -> GC screen coordinates (640x480), so interface hit tests can
 * be written against the same coordinates the game draws in. */
void pc_mouse_get_native_position(s32* x, s32* y) {
    s32 mouse_x, mouse_y;
    f32 scaled_x, scaled_y;

    pc_mouse_get_position(&mouse_x, &mouse_y);

    if (g_pc_window_w > 0 && g_pc_window_h > 0) {
        scaled_x = ((f32)mouse_x * PC_GC_WIDTH) / (f32)g_pc_window_w;
        scaled_y = ((f32)mouse_y * PC_GC_HEIGHT) / (f32)g_pc_window_h;
#ifdef PC_ENHANCEMENTS
        if (g_aspect_active) {
            /* Undo the horizontal squeeze/offset the renderer applies. */
            scaled_x = (scaled_x - g_aspect_offset) / g_aspect_factor;
        }
#endif
    } else {
        scaled_x = (f32)mouse_x;
        scaled_y = (f32)mouse_y;
    }

    if (x) *x = (s32)scaled_x;
    if (y) *y = (s32)scaled_y;
}

void pc_mouse_update(void) {
    int sdl_x = 0, sdl_y = 0;
    int rel_x = 0, rel_y = 0;
    u32 sdl_buttons;
    s32 mx, my;

    g_mouse_buttons_previous = g_mouse_buttons_current;
    g_mouse_buttons_current = 0;

    sdl_buttons = SDL_GetMouseState(&sdl_x, &sdl_y);
    mx = (s32)sdl_x;
    my = (s32)sdl_y;

    /* Convert each SDL button to our bitflag system */
    if (sdl_buttons & SDL_BUTTON(SDL_BUTTON_LEFT))   g_mouse_buttons_current |= PC_MOUSE_BUTTON_LEFT;
    if (sdl_buttons & SDL_BUTTON(SDL_BUTTON_MIDDLE)) g_mouse_buttons_current |= PC_MOUSE_BUTTON_MIDDLE;
    if (sdl_buttons & SDL_BUTTON(SDL_BUTTON_RIGHT))  g_mouse_buttons_current |= PC_MOUSE_BUTTON_RIGHT;
    if (sdl_buttons & SDL_BUTTON(SDL_BUTTON_X1))     g_mouse_buttons_current |= PC_MOUSE_BUTTON_X1;
    if (sdl_buttons & SDL_BUTTON(SDL_BUTTON_X2))     g_mouse_buttons_current |= PC_MOUSE_BUTTON_X2;

    /* Latch the wheel movement the event loop collected since the last frame.
     * Anything unread is dropped rather than carried over, so a stale scroll
     * cannot keep the mouse looking active forever. */
    g_mouse_wheel_frame = g_mouse_wheel_delta;
    g_mouse_wheel_delta = 0;

    if (g_mouse_wheel_frame > 0) {
        g_mouse_buttons_current |= PC_MOUSE_WHEEL_UP;
    } else if (g_mouse_wheel_frame < 0) {
        g_mouse_buttons_current |= PC_MOUSE_WHEEL_DOWN;
    }

    /* Relative state has to be drained every frame whether or not it is used,
     * otherwise it accumulates and the first locked frame jumps. */
    SDL_GetRelativeMouseState(&rel_x, &rel_y);

    if (g_mouse_locked) {
        g_mouse_dx = (s32)rel_x;
        g_mouse_dy = (s32)rel_y;
        g_mouse_moved = (g_mouse_dx != 0 || g_mouse_dy != 0);
    } else {
        g_mouse_dx = mx - g_mouse_x;
        g_mouse_dy = my - g_mouse_y;
        g_mouse_moved = (mx != g_mouse_x || my != g_mouse_y);
    }

    g_mouse_x = mx;
    g_mouse_y = my;

    g_mouse_active_this_frame =
        (g_mouse_moved || (g_mouse_buttons_current & ~(PC_MOUSE_WHEEL_UP | PC_MOUSE_WHEEL_DOWN)) ||
         g_mouse_wheel_frame != 0);
}

#endif /* MOUSE_INPUT */
