...
/* One entry per table, indexed by mTG_TABLE_*; adding a table must add a row. */
typedef char mTG_table_mouse_data_size_check
    [(sizeof(mTG_table_mouse_data) / sizeof(mTG_table_mouse_data[0]) == mTG_TABLE_NUM) ? 1 : -1];
...
static void mTG_mouse_move_func(Submenu* submenu, mSM_MenuInfo_c* menu_info) {
    ...
    /* Same guard mTG_move_func applies: sel_tag_idx is -1 while no tag is up. */
    ...
    f32 logical_mouse_x = (f32)mouse_x * 0.5f;
    f32 logical_mouse_y = (f32)mouse_y * 0.5f;
    ...
    if (tag_ovl->sel_tag_idx == 0) {
        if (tag_ovl->hovered_table >= 0) {
            if (mouse_pressed & PC_MOUSE_BUTTON_LEFT) { // Left click
        ...
        if (mouse_pressed & PC_MOUSE_BUTTON_RIGHT) { // Right click
            u32 old_trigger = submenu->overlay->menu_control.trigger;
