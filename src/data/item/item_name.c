#ifdef TARGET_PC
unsigned char itemName_paper[0x1000];
#else
unsigned char itemName_paper[] = {
#include "assets/itemName_paper.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_money[0x40];
#else
unsigned char itemName_money[] = {
#include "assets/itemName_money.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_tool[0x5C0];
#else
unsigned char itemName_tool[] = {
#include "assets/itemName_tool.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_fish[0x280];
#else
unsigned char itemName_fish[] = {
#include "assets/itemName_fish.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_cloth[0xFF0];
#else
unsigned char itemName_cloth[] = {
#include "assets/itemName_cloth.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_etc[0x310];
#else
unsigned char itemName_etc[] = {
#include "assets/itemName_etc.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_carpet[0x430];
#else
unsigned char itemName_carpet[] = {
#include "assets/itemName_carpet.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_wall[0x430];
#else
unsigned char itemName_wall[] = {
#include "assets/itemName_wall.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_fruit[0x80];
#else
unsigned char itemName_fruit[] = {
#include "assets/itemName_fruit.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_plant[0xB0];
#else
unsigned char itemName_plant[] = {
#include "assets/itemName_plant.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_minidisk[0x370];
#else
unsigned char itemName_minidisk[] = {
#include "assets/itemName_minidisk.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_dummy[0x100];
#else
unsigned char itemName_dummy[] = {
#include "assets/itemName_dummy.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_ticket[0x600];
#else
unsigned char itemName_ticket[] = {
#include "assets/itemName_ticket.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_insect[0x2D0];
#else
unsigned char itemName_insect[] = {
#include "assets/itemName_insect.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_hukubukuro[0x20];
#else
unsigned char itemName_hukubukuro[] = {
#include "assets/itemName_hukubukuro.inc"
};
#endif

#ifdef TARGET_PC
unsigned char itemName_kabu[0x40];
#else
unsigned char itemName_kabu[] = {
#include "assets/itemName_kabu.inc"
};
#endif

#ifdef TARGET_PC
unsigned char ftrName_table[0x4000];
#else
unsigned char ftrName_table[] = {
#include "assets/ftrName_table.inc"
};
#endif

#ifdef TARGET_PC
/* 0xF20 stock records loaded from the disc image (pc_assets), plus one 16-byte
 * record per custom furniture item filled in by pc_custom_furniture_init(). */
unsigned char ftrName2_table[0xF20 + 5 * 0x10];
#else
unsigned char ftrName2_table[] = {
#include "assets/ftrName2_table.inc"
};
#endif
