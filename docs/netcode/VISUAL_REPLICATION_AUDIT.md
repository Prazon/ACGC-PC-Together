...
| **Footprints.** *Delivered 2026-08-08.* The frame test is the game's own, exported rather than copied -- the tables are one frame list per animation. The foot positions come from the draw callback at the same joints the local player uses, and the ground attribute is looked up at the foot rather than read from a collision result the presentation actor does not have. Marks only: the footstep *sound* is remote audio and stays out with the rest. Ripples, dust and splash are still absent. | Both inputs are local: the viewer runs the remote's keyframe itself and can read the ground attribute at its position. |
| **Nameplate.** `remote->name` is replicated and never drawn. *Not wanted -- the maintainer declined it 2026-08-08.* | Pure presentation; the data is already there. |
